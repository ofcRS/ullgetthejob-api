...
export const app = new Elysia()
  ...
  .ws('/ws', {
    ...
    message(ws, message) {
      try {
        const data = JSON.parse(String(message)) as { type?: string; filters?: Record<string, unknown> }
        if (data?.type === 'subscribe') {
          const meta = clientMeta.get(ws.raw) ?? {}
          meta.filters = data.filters ?? {}
          clientMeta.set(ws.raw, meta)
          ws.send(JSON.stringify({ type: 'subscribed', filters: meta.filters }))
          return
        }
  ...
  .post('/api/v1/jobs/broadcast', ({ headers, body, set }) => {
    ...
    for (const client of clients) {
      try {
        ...
        const payload = JSON.stringify({ type: 'new_jobs', jobs: filteredJobs, stats })
        client.send(payload)
        delivered++
      } catch {}
    }
...
function filterJobsByUserPreferences(jobs: Job[], filters: Record<string, unknown>): Job[] {
