...
export class CVParserService {
  ...
  private async parsePDF(buffer: ArrayBuffer): Promise<ParsedCV> {
    try {
      ...
      return this.extractStructuredData(data.text)
    } catch (error) {
  ...
  private extractStructuredData(text: string): ParsedCV {
    ...
    return {
      ...
      experience: this.extractSection(normalizedText, ['experience', 'work history', 'employment', 'professional experience']),
      education: this.extractSection(normalizedText, ['education', 'academic', 'qualification', 'degree']),
      skills: this.extractSkills(normalizedText),
      projects: this.extractSection(normalizedText, ['projects', 'portfolio', 'personal projects'])
    }
  }
  ...
  private extractSection(text: string, keywords: string[]): string {
    ...
    const sectionKeywords = ['experience', 'education', 'skills', 'projects', 'contact', 'references']
  ...
  private extractSkills(text: string): string[] {
    ...
    const skillsSection = this.extractSection(text, ['skills', 'technologies', 'tools', 'competencies'])
    ...
    for (const pattern of skillPatterns) {
      ...
      while ((match = pattern.exec(skillsSection)) !== null) {
        ...
        if (skill.length > 2 && skill.length < 50) { // Filter reasonable skill names
          skills.push(skill)
        }
      }
    }
    ...
    const commonTechSkills = [
      ...
      'HTML', 'CSS', 'Sass', 'Less', 'Git', 'Linux', 'Windows', 'macOS'
    ]
